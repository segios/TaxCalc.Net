...
using Trade = Txc.Services.IB.Model.IBTrade;
...
namespace Txc.Services.IB.Mappings
{
    ...
    public class TradeMapping : ClassMapping<Trade>
    {
        public TradeMapping()
        {
            MapSection("Trades");
            ...
            SetFilter(x => ((Trade)x).AssetCategoryType == AssetCategoryType.Stocks ||
                        ((Trade)x).AssetCategoryType == AssetCategoryType.Options);
        }
    }
}
